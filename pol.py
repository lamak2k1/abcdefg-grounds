...
def check_password():
    ...
    def password_entered():
        ...
        if "password" in st.session_state and hmac.compare_digest(st.session_state["password"], mentor_password):
                ...
                if score_1 >= 0.7 and score_2 >= 0.7:
                    ...
                    with col1:
                        with st.container(border=True):
                            st.link_button(firsttitle.strip('"'), source_nodes[0].metadata['page_id'], help='Go to lesson', use_container_width=True)

                    with col2:
                        with st.container(border=True):
                            st.link_button(secondtitle.strip('"'), source_nodes[1].metadata['page_id'], help='Go to lesson', use_container_width=True)
